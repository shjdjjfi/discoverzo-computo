...
df=pd.read_csv(DATA/'figure_06_approximate_subspace.csv')
...
fig,ax=plt.subplots(figsize=(9,4.2)); im=ax.imshow(mat,aspect='auto'); ax.set_xticks(np.arange(len(order)),order,rotation=30,ha='right'); ax.set_yticks(np.arange(len(names)),names); ax.set_title('Median log10 regret with approximate ridge structure (tau=0.02)'); fig.colorbar(im,ax=ax,label='log10 regret'); fig.tight_layout(); fig.savefig(OUT/'figure_06_approximate_subspace.png',dpi=180)
